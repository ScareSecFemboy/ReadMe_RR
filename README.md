...
I have been asked this alot along with some other things, some people around 15+ have asked me if the project will go pro, and if I will make a way more corperate level enviroment for red rabbit. The short answer is we wont while the other answer is longer, I have decided i may make red rabbit way more useable, using languages like C++ / C# and even objective C I will end up making a web interface for red rabbits tools and even add the option to run your own scripts inside of the web UI, the reason for this is simply due to the fact that most cyber weapons do remain CLI applications, however with the way technology was built and the way it is heading it might be a smart idea to keep CLI but also add more web interface use into it. 
