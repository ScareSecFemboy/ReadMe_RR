...
func Warning_advanced(msg, color string, msg_format int, file, extra, rr6 bool, err error, exit_code int, dev_code int, filename string) bool {
	if err != nil {
		if rr6 {
			if msg_format == 1 {
				fmt.Println(color, "<RR6> ERRORS MODULE: Got dev code < ", dev_code, "> | -> ", msg, "\033[39m\033[49m")
			}
			if msg_format == 2 {
				fmt.Print(color, "<RR6> ERRORS MODULE: Got dev code < ", dev_code, "> | -> ", msg, "\033[39m\033[49m")
			}
			if msg_format == 3 {
				log.Println(color, "<RR6> ERRORS MODULE: Got dev code < ", dev_code, "> | -> ", msg, "\033[39m\033[49m")
			}
		} else {
			if file {
				if msg_format == 1 {
...
func Ce(err error, color, msg string, exit_code int) {
	if err != nil {
		fmt.Println(color, msg, "\033[39m\033[49m")
		os.Exit(0)
	}
}
