...
func LookHead(url string) {
	...
	for name, val := range response.Header {
		fmt.Println(v.REDHB, "Header -> \033[49m|", v.BLKHB, name, "\033[49m\033[39m|")
		fmt.Println(v.BLKHB, "| =====> \033[49m\033[39m", v.BLKHB, val, "\033[49m\033[39m")
		system.Sep("\n")
	}
}

func LookSRV(hostname, form, server_t string) {
	// form      : tcp
	// hostname  : google.com
	// servert   : xmpp-server
	cname, srvs, err := net.LookupSRV(server_t, form, hostname)
	if err != nil {
		log.Fatal(err)
	}
	ec.Warning_advanced("<RR6> Lookup Module: Could not make a direct SRV record lookup, something went wrong -> ", v.REDHB, 1, false, false, true, err, 1, 233, "")
...
func LookNS(hostname string) {
	...
	for _, nameserver := range nS {
		fmt.Println(v.BLKHB, " < Name Servers > |> \033[49m\033[31m", nameserver.Host)
	}
}
...
func LookMX(domain string) {
	...
	for _, mxRecord := range mxRecords {
		fmt.Printf("\033[1;100m\033[31m< MX Host > |> \033[49m\033[31m %s\t\033[1;100m\033[31m< MX Preference > |> \033[49m\033[31m%d\n", mxRecord.Host, mxRecord.Pref)
	}
...
func LookHSIP(hostname string) {
	...
	for _, ipa := range ip {
		fmt.Println(v.BLKHB, "< IPA > |> \033[49m\033[31m ", ipa)
	}
}
...
func LookIP(ipa string) {
	...
	for _, hostnames := range hostnames {
		fmt.Println(v.BLKHB, "< Hostnames > |> \033[49m\033[31m ", hostnames)
	}
}
...
func Lookup_banner_main(IPADDR string) {
	...
	for port := 0; port <= 1024; port++ {
		...
		ac_threads++
		fmt.Println(v.BLKHB, "<RR6> Concurrency Module: Started threads | ", ac_threads, " | ", v.RET_RED)
	}
	for ac_threads > 0 {
		<-cancel_chan
		ac_threads--
...
func LookBANNER(p int, h string, cancel_chan chan bool) {
	...
	if e != nil {
		...
		if ex != nil {
			...
			fmt.Printf("[+] Got banner from port | %d | banner - | %s | ", p, buf[0:br])
			cancel_chan <- true
		}
	}
...
func Mac_trace(mac string) {
	...
	if err != nil {
		fmt.Println("Mac -> ", mac, " | Was not found for an address")
	}
	fmt.Println(mac, data.Location.Longitude, data.Location.Latitude)
}
...
func sighandel(c chan os.Signal) {
	...
	for s := <-c; ; s = <-c {
		...
		case os.Kill:
			fmt.Println("\n\n\tKILL received")
			fmt.Println("\033[39m\033[49m[+] Returning color....")
			os.Exit(0)
		}
	}
}
...
func host_miner(p string, chann chan bool) {
	...
	if ex != nil {
		fmt.Println(v.BLKHB, "<RR6> No host found, skipping host ~~>  ", p, v.RET_RED)
		fmt.Println("-----------------------------------------------------------------------------")
		chann <- true

	} else {
		fmt.Printf("<RR6> Lookup Module: Found addr | %s to -> %s\n", p, strings.Join(a, ", "))
	}
}
