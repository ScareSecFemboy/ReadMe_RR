...
func brute_s(wl, user, dbtype, host, dbname string) {
	...
	fmt.Println("[+] Setting: Got return code -> ", pass)
...
func Parse_Server(username, wordlist, hostname, dbname, dbtype string) {
	...
	fmt.Println("[+] Setting...: Wordlist -> ", wordlist)
	fmt.Println("[+] Setting...: DB Name  -> ", dbname)
	fmt.Println("[+] Setting...: DB Type  -> ", dbtype)
	fmt.Println("[+] Setting...: DB Host  -> ", hostname)
