...
func (Image_metachunk *MetaChunk) Run_Functions(image_inject bool, b *bytes.Reader, c *rr6_opts.RR6_options, scan_meta bool) {
	...
	if image_inject {
		fmt.Println("[+] Setting: Modifying Payload         | ", c.Payload)
		m.Chk.Chunk_Data = []byte(c.Payload)
		fmt.Println("[+] Setting: Modifying Type            | ", c.Type)
		m.Chk.Chunk_Type = m.strToInt(c.Type)
		fmt.Println("[+] Setting: Modifying Chunk Size      | ", m.Chk.Chunk_Size)
		m.Chk.Chunk_Size = m.CCS()
		m.Chk.Chunk_CRC = m.CCCRC()
		bm := m.marshalData()
		fmt.Println("[+] Setting: Modifying data            | ")
		bmb := bm.Bytes()
		fmt.Printf("[+] Setting: Untouched/Unmodded payload |  % X\n", []byte(c.Payload))
		fmt.Printf("[+] Setting: Untouched/Unmodded Byte P  |  % X\n", m.Chk.Chunk_Data)
		utils.J(b, c, bmb)
	}
